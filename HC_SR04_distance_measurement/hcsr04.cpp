               ...
               uint8_t gpioAFSelTIMx)
{
    ...
    htim.Instance = TIMx;
    htim.Init.Prescaler = 84 - 1; // 1 MHz timer clock assuming 84 MHz system clock
    htim.Init.CounterMode = TIM_COUNTERMODE_UP;
...
{
    const uint32_t sysClockFreq = 84000000; // Adjusted for STM32F4
    uint32_t statusReg = htim.Instance->SR;

    if ((statusReg & TIM_SR_CC2IF) == TIM_SR_CC2IF)
    {
        pulseWidth = htim.Instance->CCR2;
        distanceCM = (float)pulseWidth * 1000000 / (58 * sysClockFreq / (htim.Init.Prescaler + 1));
    }
    return distanceCM;
}
